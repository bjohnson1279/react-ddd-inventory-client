...
export const CycleCountDashboardPanel: React.FC<{ tenantId: string }> = ({
  tenantId,
}) => {
        ...
        {counts.length === 0 ? (
          ...
          <div className="space-y-4">
            {counts.map((count) => (
              <div
                key={count.id}
                className="flex justify-between items-center p-4 rounded-lg border border-slate-100 hover:bg-slate-50 transition-colors"
              >
                <div>
                ...
                <button
                  ...
                  disabled={submittingId === count.id}
                  aria-busy={submittingId === count.id}
                >
                  {submittingId === count.id ? (
                    <Spinner />
                  ) : (
                    <>
