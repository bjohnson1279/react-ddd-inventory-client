...
export const ApprovalWorkflowPanel: React.FC<ApprovalWorkflowPanelProps> = ({ api }) => {
    ...
    <div style={{ padding: '24px', background: '#0f172a', color: '#f8fafc', borderRadius: '12px', minHeight: '600px' }}>
      <div style={{ borderBottom: '1px solid #334155', paddingBottom: '16px', marginBottom: '24px' }}>
        ...
        <p style={{ color: '#94a3b8', marginTop: '4px', fontSize: '14px' }}>
          Define multi-step approval chains for sensitive domain actions like Purchase Order placement or Inventory write-offs.
        </p>
      </div>
      ...
      {loading && workflows.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {workflows.map(wf => (
            <div key={wf.id} style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '8px', padding: '16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                <h3 style={{ margin: 0, fontSize: '18px', color: '#e2e8f0' }}>{wf.triggerEvent}</h3>
                <button 
              ...
              <div style={{ marginTop: '12px' }}>
                <h4 style={{ margin: '0 0 8px 0', fontSize: '14px', color: '#94a3b8' }}>Approval Steps</h4>
                {wf.steps.length > 0 ? (
                  <ol style={{ paddingLeft: '20px', margin: 0, color: '#cbd5e1' }}>
                    {wf.steps.map((step, i) => (
                      <li key={i} style={{ marginBottom: '4px' }}>
                        Role: <strong>{step.roleId}</strong> | Requires: {step.minApprovals} approval(s)
                      </li>
                    ))}
                  </ol>
                ) : (
                  <p style={{ margin: 0, color: '#64748b', fontSize: '14px' }}>No steps configured.</p>
                )}
              </div>
            </div>
          ))}
          ...
          {workflows.length === 0 && (
            <div style={{ padding: '24px', textAlign: 'center', color: '#94a3b8', background: '#1e293b', borderRadius: '8px', border: '1px dashed #334155' }}>
              No approval workflows configured yet.
              <br/>
              <button style={{ marginTop: '12px', padding: '8px 16px', background: '#38bdf8', color: '#0f172a', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' }}>
                Create New Workflow
              </button>
            </div>
          )}
